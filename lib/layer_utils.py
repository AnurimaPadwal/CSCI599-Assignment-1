...
class fc(object):
    ...
    def backward(self, dprev):
        ...
        self.grads[self.w_name] = np.dot(np.reshape(feat, (feat.shape[0], np.prod(feat.shape[1:]) )).T, dprev)
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        log_probability =np.log(logit[range(N), label])
        if self.size_average == True:
            loss = -(np.sum(log_probability))/N
        else:
    ...
    def backward(self):
        ...
        logit[np.arange(label.shape[0]), label]-=1
        if self.size_average == True:
